...
def find_favorite(array_of_hash_objects)
  ...
  # Didnt work
  # array_of_hash_objects.each { |hash| hash[:name] if hash[:is_my_favorite?] }

  # It returns an array with the hash that I need, so to just return the hash I
  # have it return element 0.  I feel like there is a better way to do this, and
  # that I missed something in either the lesson or ruby docs
  array_of_hash_objects.select{ |hash| hash[:is_my_favorite?]}[0]
end
